...
export async function register_user(req, res, next) {
  if (!req.body || !req.body.email || !req.body.password || !req.body.email) {
  ...
  const newobject = {
    ...
    password,
    email,
    avatar: { public_profile: "test", public_url: "test" },
    role,
  };
...
export async function login_user(req, res, next) {
  ...
  if (!result_user) {
    return next(new custom_error("Email or password Incorrect", 401));
  }
  ...
  return res
    ...
    .send({ msg: "success", result_user, token });
}
...
export async function password_reset(req, res, next) {
  ...
  console.log(
    "token url",
    `${req.protocol}${req.domain}/password/reset/${token}`
  );
